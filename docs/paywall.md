...
Gate the "Crazy Ideas (Viral Hooks)" features behind a one-time payment. Local-first architecture — no backend DB needed for license management.
...
| Free                           | Premium (one-time Rs.149-249)              |
| ------------------------------ | ------------------------------------------ |
| Manual + voice expense logging | Kal Ka Tu (daily AI roast)                 |
| Basic summary view             | Kharcha Catchphrase (per-txn wit)          |
| Recurring tracking             | Kitna Diya (price comparison)              |
| Household sync                 | Kharcha Wrapped (shareable cards)          |
| Snap Streak (drives retention) | Rs.1 Challenge (shareable challenge cards) |

> Keep Snap Streak free — it drives daily opens which increases conversion to premium.
