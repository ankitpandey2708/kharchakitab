...
const TrendChart = memo(
    ({
        ...
        // PERF-HANDLER: Throttled mouse move handler using requestAnimationFrame
        const handleMouseMove = useCallback((event: React.MouseEvent) => {
            if (rafId.current) return; // Skip this frame if already scheduled
        ...
        // PERF-HANDLER: Throttled touch move handler using requestAnimationFrame
                ...
                >
                    ...
                    {totalPrev > 0 && (
                        <text
                            ...
                            style={{ paintOrder: "stroke", stroke: "white", strokeWidth: 3 }}
                        >
                            Total {currencySymbol}{formatCurrency(totalPrev)}
                        </text>
                    )}
                    <path
                ...
                {hoverIndex !== null && chartMeta.points[hoverIndex] && (
                    ...
                    >
                        <div className="kk-micro font-semibold text-[var(--kk-ink)]">
                            <span className="kk-currency">{currencySymbol}</span>{formatCurrency(chartMeta.points[hoverIndex].value)}
                        </div>
                        <div>
                            {bucket === "hour"
                                ...
                                allocationMetrics.topCategories.map((category, index) => {
                                    ...
                                    const absoluteValue =
                                        allocationMode === "count"
                                            ...
                                            : `${currencySymbol}${formatCurrency(category.total)}`;
                                    return (
