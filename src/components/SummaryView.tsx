...
import { Mic, ArrowDown, Sparkles, Users } from "lucide-react";
import {
  deleteTransaction,
  fetchTransactions,
  updateTransaction,
...
export const SummaryView = React.memo(({
  refreshKey,
  onViewAll,
  onEdit,
  onDeleted,
  ...
  const reloadTransactions = useCallback((isActive?: () => boolean) => {
    ...
    const recentPromise = isCurrentMonth
      ...
      : fetchTransactions({ limit: 5, before: beforeNow })
        .then((items) => {
  ...
  const householdCategories = useMemo(() => {
    ...
    for (const tx of householdData.transactions) {
      if (isProcessingRow(tx)) continue;
