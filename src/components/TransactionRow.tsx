  ...
  ({
    ...
    return (
      <motion.div
        ...
        transition={{ duration: 0.28, delay: Math.min(index * 0.035, 0.15) }}
        // PERF-ANIMATION: Removed transition-colors and transition-shadow, using transform only
        className={`group relative flex items-center justify-between gap-3 overflow-hidden kk-radius-md border border-[var(--kk-smoke)] p-4 pl-5 pr-4 bg-white transform-gpu will-change-transform ${isClickable ? "cursor-pointer active:scale-[0.995]" : ""
          }`}
            ...
            {isProcessing ? (
              ...
              <>
                ...
                <div className="mt-0.5 flex items-center gap-2 kk-meta flex-nowrap overflow-hidden">
                  ...
                  {ownerLabel && tx.owner_device_id && (
                    <span className={`kk-pill ${ownerLabel === "Me" ? "bg-[var(--kk-ember)]/10 text-[var(--kk-ember)]" : "kk-pill-muted"}`}>
                      {ownerLabel === "Me" ? <Fingerprint className="h-3 w-3" /> : <Handshake className="h-3 w-3" />}
                    </span>
                  )}
                  {tx.is_private && (
                    <span
        ...
        <div className="flex min-w-0 shrink items-center gap-1.5 sm:gap-2">
          {isProcessing ? (
            ...
            <div className="flex flex-col items-end gap-0.5">
              ...
              {tx.amortized && tx.recurring_total_amount != null && tx.recurring_frequency && (
                <div className="whitespace-nowrap text-[10px] text-[var(--kk-ash)]">
                  of <span className="kk-currency">{currencySymbol}</span>{formatCurrency(tx.recurring_total_amount)}/{tx.recurring_frequency === "yearly" ? "yr" : "qtr"}
                </div>
              )}
            </div>
          )}
            ...
            <>
              ...
              {!isProcessing && (
                onOpenMobileSheet ? (
                  ...
                  <div className="h-8 w-8 flex-none kk-mobile-only" aria-hidden="true" />
                )
              )}
            </>
          )}
