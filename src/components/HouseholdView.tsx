// PERF-VIRTUAL: Added virtualization for large transaction lists
...
import { RefreshCw, Shield, Users, X, AlertCircle, Smartphone, ArrowRight, Check, Wifi, WifiOff, Clock, ChevronRight, TrendingUp, Pencil, Fingerprint, Handshake } from "lucide-react";
import {
  clearConflict,
  getDeviceIdentity,
  getPairings,
...
export const HouseholdView = React.memo(() => {
  ...
  const virtualizer = useVirtualizer({
    count: filteredTransactions.length > 50 ? filteredTransactions.length : 0,
    getScrollElement: () => virtualListRef.current,
    estimateSize: () => 72, // Approximate row height
    overscan: 5,
    enabled: filteredTransactions.length > 50,
  });
      ...
      <div className="grid gap-4 lg:grid-cols-12 lg:gap-6">
        ...
        <div className="order-1 space-y-4 lg:order-1 lg:col-span-8">
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--kk-smoke)] bg-white p-5 shadow-md">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-3xl font-bold text-[var(--kk-ink)]">
                  <span className="kk-currency">{currencySymbol}</span>{formatCurrency(totals.total)}
                </div>
              </div>
            ...
            {/* Split Bar - PERF-ANIMATION: Using transform: scaleX instead of width for GPU acceleration */}
            <div className="mt-4 mb-3 flex h-3 w-full overflow-hidden rounded-full bg-[var(--kk-smoke-heavy)]">
              <div
                className="h-full origin-left bg-gradient-to-r from-[var(--kk-ocean)] to-[#5b7dea] transition-transform duration-700 ease-out will-change-transform"
                style={{ transform: `scaleX(${totals.youPct / 100})`, width: '100%' }}
              />
            </div>
            {/* Hidden second bar for partner percentage - using separate div to avoid layout issues */}
            <div className="hidden">
              <div
                className="h-full origin-left bg-gradient-to-r from-[var(--kk-ember)] to-[var(--kk-saffron)] transition-transform duration-700 ease-out will-change-transform"
                style={{ transform: `scaleX(${(100 - totals.youPct) / 100})`, width: '100%' }}
              />
            </div>

            <div className="flex justify-between text-xs">
              <span className="flex items-center gap-2 text-[var(--kk-ink)]">
                <Fingerprint className="h-3.5 w-3.5 text-[var(--kk-ash)]" />
                <span className="font-medium">Me</span>
                <span className="font-mono text-[var(--kk-ash)]">{currencySymbol}{formatCurrency(totals.you)}</span>
              </span>
              <span className="flex items-center gap-2 text-[var(--kk-ink)]">
                <span className="font-mono text-[var(--kk-ash)]">{currencySymbol}{formatCurrency(totals.partner)}</span>
                <span className="font-medium">Partner</span>
                <Handshake className="h-3.5 w-3.5 text-[var(--kk-ash)]" />
              </span>
            </div>
          </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {conflictVersions.map((version, i) => (
                  <div key={i} className="rounded-xl border border-[var(--kk-smoke-heavy)] p-4 hover:border-[var(--kk-ember)] transition-colors bg-white">
                    ...
                    <div className="font-mono font-bold text-xl text-[var(--kk-ink)]"><span className="kk-currency">{currencySymbol}</span>{formatCurrency(version.snapshot.amount)}</div>
                    <div className="mt-4 flex items-center gap-1.5 text-xs text-[var(--kk-ash)] mb-3">
                      Edited by
                      {version.editorId === identity?.device_id
                        ? <><Fingerprint className="h-3 w-3 inline" /> Me</>
                        : <><Handshake className="h-3 w-3 inline" /> {partnerNameById.get(version.editorId) || "Partner"}</>}
                    </div>
          ...
          {/* PROGRESS BAR (Floating) - PERF-ANIMATION: Using transform: scaleX instead of width */}
          {syncProgress && (
            <div className="rounded-xl bg-[var(--kk-ink)] p-4 text-white shadow-lg animate-fade-up">
              ...
              <div className="h-2 w-full overflow-hidden rounded-full bg-white/20">
                <div
                  className="h-full origin-left bg-[var(--kk-ember)] transition-transform duration-300 will-change-transform"
                  style={{ transform: `scaleX(${syncProgress.current / syncProgress.total})`, width: '100%' }}
                />
              </div>
              ...
              <div className="relative z-10 sm:flex sm:items-center sm:justify-between">
                ...
                <div className="flex rounded-xl border border-[var(--kk-smoke)] bg-[var(--kk-paper)] p-1">
                  {(["all", "you", "partner"] as const).map((filter) => (
                    ...
                    >
                      {filter === "you" && <Fingerprint className="h-3.5 w-3.5" />}
                      {filter === "partner" && <Handshake className="h-3.5 w-3.5" />}
                      {filter === "all" ? "All" : filter === "you" ? "Me" : "Partner"}
                    </button>
                  ))}
                </div>
              </div>
                          ...
                          >
                            ...
                            {isPaired && (
                              <button
                                ...
                                className={`kk-icon-btn kk-icon-btn-ghost h-8 w-8 sm:h-9 sm:w-9 sm:opacity-0 sm:group-hover:opacity-100 ${confirmForgetId === device.device_id
                                  ? "text-[var(--kk-ember)]"
                                  : "kk-icon-btn-danger"
                                  }`}
                              >
