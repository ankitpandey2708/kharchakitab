...
/**
 * One authenticated POST per tool call, no `initialize` handshake.
 *
 * Swiggy's rate-limit docs warn against re-handshaking per call and prescribe
 * reusing one session — but a live `initialize` against `/food` returned no
 * `Mcp-Session-Id`, so the server issues no session to carry. Handshaking would
 * add an auth event per cold start and buy nothing.
 */
function callTool(
  token: string,
  mcpUrl: string,
