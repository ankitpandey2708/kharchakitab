import { ExpenseArraySchema, type Expense } from "@/src/utils/schemas";
...
export const parseWithGeminiFlash = async (text: string, currencyCode: CurrencyCode = "INR"): Promise<Expense[]> => {
  ...
  // Backfill date on any item missing it
  if (Array.isArray(parsed)) {
    for (const item of parsed) {
      if (item && typeof item === "object" && !("date" in item)) {
        (item as { date?: string }).date = today;
      }
    }
  }

  const result = ExpenseArraySchema.safeParse(parsed);
  if (!result.success) {
